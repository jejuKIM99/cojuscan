...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const translations = {
        ko: {
            ...
            startUrlScan: "URL 분석", 
            urlScanTitle: "URL 취약점 분석",
            urlScanInfoTitle: "주의사항",
            urlScanInfoCase1Title: "✅ 분석 가능한 경우",
            urlScanInfoCase1Desc: "HTML, CSS, JavaScript 파일들이 서버에 개별 파일로 배포된 정적 웹사이트. (예: 순수 HTML/CSS/JS, Jekyll, Hugo 기반 사이트)",
            urlScanInfoCase2Title: "⚠️ 부분적으로 분석 가능한 경우",
            urlScanInfoCase2Desc: "리액트(React), Vue 등 빌드 과정을 거치는 사이트. 이 경우 빌드된 최종 JavaScript 파일 하나만 분석됩니다. 원본 소스 코드를 분석하려면 '디렉토리 선택' 기능을 사용하세요.",
            urlScanInfoReasonTitle: "이유",
            urlScanInfoReasonDesc: "URL 분석은 실제 사용자가 브라우저로 보는 배포된 파일들만 접근할 수 있습니다. 개발용 소스코드(`src` 폴더 등)는 서버에 배포되지 않으므로 URL로 직접 접근하여 분석할 수 없습니다.",
            urlScanStep1: "1단계: 분석 환경 준비",
            urlScanSubStep1Title: "메타태그 삽입",
            urlScanSubStep1Desc: "분석할 사이트의 &lt;head&gt; 태그 안에 아래 메타 태그를 추가하세요.",
            urlScanSubStep2Title: "cojuscan.js 생성",
            urlScanSubStep2Desc: "사이트의 루트 경로에 아래와 같이 폴더와 파일을 생성하세요.",
            urlScanSubStep3Title: "파일 경로 추가 및 배포",
            urlScanSubStep3Desc: "생성한 cojuscan.js 파일에 분석할 모든 파일의 경로를 추가하세요. 아래 형식들을 지원합니다.",
            copied: "복사되었습니다!", 
            urlScanStep2: "2단계: URL 입력 및 검사", 
            urlScanStep2Desc: "사이트 배포가 완료되었다면, 분석할 URL을 입력하고 검사를 시작하세요. (예: https://example.com)",
            startScanAction: "검사 시작", 
            verificationFailed: "소유권 확인 실패. 메타 태그가 정확히 삽입되었는지 확인하세요.", 
            urlFetchFailed: "URL을 가져올 수 없습니다. 주소를 확인하거나 서버 상태를 점검하세요.", 
            invalidUrl: "유효한 URL을 입력해주세요. (https://로 시작)",
            cojuscanJsNotFound: "`cojuscan/cojuscan.js` 파일을 찾을 수 없습니다. 파일이 정확한 위치에 있고, 서버에서 접근 가능한지 확인하세요.",
            verifiedUrlListTitle: "검증된 URL 목록", noVerifiedUrls: "검증된 URL이 없습니다.", rescan: "재검사", urlScanResourcesTitle: "URL 분석 리소스", noAnalyzedResources: "분석된 리소스가 없습니다.",
            themeVarGroups: {
                "제목 표시줄": ['--title-bar-bg', '--title-bar-text-color', '--title-bar-version-text-color', '--title-bar-button-color', '--title-bar-button-hover-bg', '--title-bar-close-button-hover-bg', '--title-bar-close-button-hover-color'],
                "배경 & 테두리": ['--main-bg', '--secondary-bg', '--border-color'],
                "텍스트 & 링크": ['--text-color', '--text-color-dark', '--accent-color', '--link-color'],
                "기본 버튼": ['--button-primary-bg', '--button-primary-hover-bg', '--button-secondary-bg', '--button-secondary-hover-bg', '--button-danger-bg', '--button-danger-hover-bg'],
                ...
                "URL 검사 버튼": ['--button-url-scan-bg', '--button-url-scan-hover-bg', '--button-url-scan-text'],
                "URL 모달": ['--url-modal-border-color', '--url-modal-header-color', '--url-modal-progress-bar-bg'],
                "UI 컴포넌트": ['--progress-bar-bg', '--scrollbar-thumb-color', '--scrollbar-track-color', '--severity-high-color', '--severity-medium-color', '--severity-low-color', '--code-bg', '--code-text'],
                "도움말 모달": ['--help-modal-text-color', '--help-modal-details-bg', '--help-modal-details-text-color'],
                "상태 & 기타": ['--button-disabled-bg', '--button-disabled-text', '--input-bg', '--button-primary-text'],
            },
            themeVarNames: {
                ...
                '--help-modal-text-color': '도움말 모달 텍스트', '--help-modal-details-bg': '도움말 설명 배경', '--help-modal-details-text-color': '도움말 설명 텍스트',
                '--title-bar-bg': '제목 표시줄 배경', '--title-bar-text-color': '제목', '--title-bar-version-text-color': '버전 텍스트', '--title-bar-button-color': '버튼 아이콘', '--title-bar-button-hover-bg': '버튼 배경(호버)', '--title-bar-close-button-hover-bg': '닫기 버튼 배경(호버)', '--title-bar-close-button-hover-color': '닫기 버튼 아이콘(호버)',
                '--button-url-scan-bg': 'URL 검사 버튼', '--button-url-scan-hover-bg': 'URL 검사 (호버)', '--button-url-scan-text': 'URL 검사 텍스트',
                '--url-modal-border-color': 'URL 모달 테두리', '--url-modal-header-color': 'URL 모달 헤더', '--url-modal-progress-bar-bg': 'URL 모달 진행률 바',
            }
        },
        en: {
            ...
            // URL Scan Translations
            startUrlScan: "URL Scan", 
            urlScanTitle: "URL Vulnerability Scan",
            urlScanInfoTitle: "Please Note",
            urlScanInfoCase1Title: "✅ Fully Scannable Cases",
            urlScanInfoCase1Desc: "Static websites where HTML, CSS, and JavaScript files are deployed individually. (e.g., pure HTML/CSS/JS, Jekyll, or Hugo-based sites).",
            urlScanInfoCase2Title: "⚠️ Partially Scannable Cases",
            urlScanInfoCase2Desc: "Sites that go through a build process, like React or Vue. In this case, only the final bundled JavaScript file will be analyzed. To analyze the original source code, please use the 'Select Directory' feature.",
            urlScanInfoReasonTitle: "The Reason",
            urlScanInfoReasonDesc: "URL scanning can only access files that are publicly deployed and visible to a user's browser. Development source code (like the `src` folder) is not deployed to the server and thus cannot be accessed directly via a URL.",
            urlScanStep1: "Step 1: Prepare for Analysis", 
            urlScanSubStep1Title: "Insert Meta Tag",
            urlScanSubStep1Desc: "Add the meta tag below to your site's &lt;head&gt; tag.",
            urlScanSubStep2Title: "Create cojuscan.js",
            urlScanSubStep2Desc: "Create the folder and file in your site's root directory as shown below.",
            urlScanSubStep3Title: "Add File Paths & Deploy",
            urlScanSubStep3Desc: "In the created cojuscan.js file, add the paths of all files to be analyzed. The following formats are supported.",
            copied: "Copied!", 
            urlScanStep2: "Step 2: Enter URL and Scan", 
            urlScanStep2Desc: "Once the site is deployed, enter the URL to analyze and start the scan. (e.g., https://example.com)", 
            startScanAction: "Start Scan", 
            verificationFailed: "Ownership verification failed. Please ensure the meta tag is inserted correctly.", 
            urlFetchFailed: "Could not fetch the URL. Check the address or server status.", 
            invalidUrl: "Please enter a valid URL (starting with https://).",
            cojuscanJsNotFound: "Could not find `cojuscan/cojuscan.js`. Please ensure the file is in the correct location and accessible on the server.",
            verifiedUrlListTitle: "Verified URL List", noVerifiedUrls: "No verified URLs.", rescan: "Re-scan", urlScanResourcesTitle: "URL Scan Resources", noAnalyzedResources: "No resources analyzed.",
            themeVarGroups: {
                "Title Bar": ['--title-bar-bg', '--title-bar-text-color', '--title-bar-version-text-color', '--title-bar-button-color', '--title-bar-button-hover-bg', '--title-bar-close-button-hover-bg', '--title-bar-close-button-hover-color'],
                "Background & Borders": ['--main-bg', '--secondary-bg', '--border-color'],
                "Text & Links": ['--text-color', '--text-color-dark', '--accent-color', '--link-color'],
                "Standard Buttons": ['--button-primary-bg', '--button-primary-hover-bg', '--button-secondary-bg', '--button-secondary-hover-bg', '--button-danger-bg', '--button-danger-hover-bg'],
                ...
                "URL Scan Buttons": ['--button-url-scan-bg', '--button-url-scan-hover-bg', '--button-url-scan-text'],
                "URL Modal": ['--url-modal-border-color', '--url-modal-header-color', '--url-modal-progress-bar-bg'],
                "UI Components": ['--progress-bar-bg', '--scrollbar-thumb-color', '--scrollbar-track-color', '--severity-high-color', '--severity-medium-color', '--severity-low-color', '--code-bg', '--code-text'],
                "Help Modal": ['--help-modal-text-color', '--help-modal-details-bg', '--help-modal-details-text-color'],
                "State & Misc": ['--button-disabled-bg', '--button-disabled-text', '--input-bg', '--button-primary-text'],
            },
            themeVarNames: {
                '--main-bg': 'Main BG', '--secondary-bg': 'Secondary BG', '--accent-color': 'Accent', '--text-color': 'Text', '--text-color-dark': 'Muted Text', '--border-color': 'Border', '--button-primary-bg': 'Primary Button', '--button-primary-hover-bg': 'Primary Button (Hover)', '--button-primary-text': 'Primary Button Text', '--button-secondary-bg': 'Secondary Button', '--button-secondary-hover-bg': 'Secondary Button (Hover)', '--button-scan-simple-bg': 'Simple Scan Button', '--button-scan-simple-hover-bg': 'Simple Scan (Hover)', '--button-scan-precision-bg': 'Precision Scan Button', '--button-scan-precision-hover-bg': 'Precision Scan (Hover)', '--button-scan-text': 'Scan Button Text', '--button-danger-bg': 'Danger Button', '--button-danger-hover-bg': 'Danger Button (Hover)', '--button-disabled-bg': 'Disabled Button', '--button-disabled-text': 'Disabled Button Text', '--input-bg': 'Input BG', '--progress-bar-bg': 'Progress Bar', '--scrollbar-thumb-color': 'Scrollbar Thumb', '--scrollbar-track-color': 'Scrollbar Track', '--severity-high-color': 'Severity (High)', '--severity-medium-color': 'Severity (Medium)', '--severity-low-color': 'Severity (Low)', '--code-bg': 'Code Block BG', '--code-text': 'Code Block Text', '--link-color': 'Link Color',
                 '--help-modal-text-color': 'Modal Text', '--help-modal-details-bg': 'Modal Details BG', '--help-modal-details-text-color': 'Modal Details Text',
                 '--title-bar-bg': 'Title Bar BG', '--title-bar-text-color': 'Title Text', '--title-bar-version-text-color': 'Version Text', '--title-bar-button-color': 'Button Icon', '--title-bar-button-hover-bg': 'Button BG (Hover)', '--title-bar-close-button-hover-bg': 'Close BG (Hover)', '--title-bar-close-button-hover-color': 'Close Icon (Hover)',
                 '--button-url-scan-bg': 'URL Scan Button', '--button-url-scan-hover-bg': 'URL Scan (Hover)', '--button-url-scan-text': 'URL Scan Text',
                 '--url-modal-border-color': 'URL Modal Border', '--url-modal-header-color': 'URL Modal Header', '--url-modal-progress-bar-bg': 'URL Modal Progress',
            }
        }
    };
    let currentLang = 'ko';
    ...
    async function initializeApp() {
        try {
            ...
            const savedVerifiedUrls = await window.electronAPI.getSetting('verifiedUrls');
